      ...
      바이야:
        ...
        복장.${상의|하의|속옷|양말|신발|장신구}:
          check:
            ...
            - 하쿠아의 심리 상태와 <user>에 대한 관심 정도를 반영
        칭호:
          type: |-
            {
              [칭호명: string]: {
          ...
          check:
            ...
            - 칭호는 히쿠아의 현재 의존 상태와 심리적 발전을 반영해야 함
...
=== "한국어 프롬프트"
    ...
    변수 출력 형식:
      rule:
        - 다음 답장의 맨 끝에 업데이트 분석과 실제 업데이트 명령을 한 번에 출력해야 합니다.
        - 업데이트 명령은 **JSON Patch (RFC 6902)** 표준과 유사하게 작동하며, 작업 객체를 포함하는 유효한 JSON 배열이어야 합니다. 단, 표준 대신 다음 작업을 지원합니다:
          - replace: 기존 경로의 변수 값 교체
          - delta: 변동 값(delta)을 사용하여 기존 숫자 변수의 값 업데이트
          - insert: 객체나 배열에 새 항목 삽입 (배열 인덱스로 `-` 사용 시 끝에 추가를 의미)
          - remove: 제거
        - `_`로 시작하는 필드 이름은 읽기 전용이므로 업데이트하지 마십시오. (예: `_변수`)
      format: |-
        <UpdateVariable>
        <Analysis>$(영어로 출력, 80단어 이하)
        - ${지난 시간 계산: ...}
        - ${현재 줄거리가 충분히 특수한지, 시간 경과가 정상 범위를 훨씬 초과했는지에 따라 변수 값의 극적인 변화 허용 여부 판단: yes/no}
        - ${변수에 대응하는 `check`를 기반으로, 이전 줄거리가 아닌 현재 답장에만 근거하여 각 변수의 업데이트 필요 여부 분석: ...}
        </Analysis>
        <JSONPatch>
        [
          { "op": "replace", "path": "${/변수/경로}", "value": "${새_값}" },
          { "op": "delta", "path": "${/숫자/변수/경로}", "value": "${양수_또는_음수의_변동값}" },
...
=== "AI 출력 예시"
    ...
    - Special Case? No, routine plot progression, no dramatic time skips.
    - 하쿠아.의존도: Baiya showed a strong reaction to Qingkong Li (trembling, calling him "brother", self-denial but accepting the candy secretly). She definitely noticed his attention and action. This indicates a significant emotional impact, warranting an increase.
    - 하쿠아.칭호: The "오빠" slip-up and the immediate self-punishment reinforce her internal conflict, but doesn't necessarily grant a new title yet. Her current titles are still very relevant.
    - 주인공.인벤토리: The mints were placed on Baiya's desk. The item should be removed from inventory.
    - 월드.최근사건: "전학생 적응" is ongoing but the specific interaction happened. "점심시간 임박" is becoming more relevant as break continues.
    </Analysis>
    <JSONPatch>
    [
      { "op": "replace", "path": "/월드/현재시간", "value": "2024-04-08 10:57" },
      { "op": "replace", "path": "/하쿠아/의존도", "value": 40 },
      { "op": "remove", "path": "/주인공/인벤토리/박하사탕" }
    ]
    </JSONPatch>
    </UpdateVariable>
    ```
...
우리는 AI가 변수 업데이트에 대한 분석을 먼저 출력하게 하고, 그 뒤에 실제 변수 업데이트 명령을 출력하게 합니다. 이처럼 AI가 먼저 "**초안을 작성(打草稿)**"한 뒤 정식 내용을 출력하게 하는 방식이 바로 **생각의 사슬(CoT)**의 전형적인 응용 사례입니다.
...
=== "영문 프롬프트"
    ...
    **스토리에 따른 급격한 변수 변화 허용**
    ```text
    - ${decide whether dramatic updates are allowed as it's in a special case or the time passed is more than usual: yes/no}
    ```

    **변수 업데이트에 필요한 [check]{.badge}를 상기하고, 이에 따라 업데이트 방법 분석**
    ```text
    - ${analyze every variable based on its corresponding `check`, according only to current reply instead of previous plots: ...}
    ```
...
=== "한국어 프롬프트"
    ...
    **스토리에 따른 급격한 변수 변화 허용 (예: 호감도가 100에서 0으로 바로 떨어짐)**
    ```text
    - ${현재 줄거리가 충분히 특수한지, 시간 경과가 정상 범위를 훨씬 초과했는지에 따라 변수 값의 극적인 변화 허용 여부 판단: yes/no}
    ```
    ...
    **변수 업데이트에 필요한 [check]{.badge}를 상기하고, 이에 따라 업데이트 방법 분석**
    ...
    그중 가장 배울 만한 점은 [변수 업데이트에 필요한 check를 상기하고, 이에 따라 업데이트 방법 분석]{.badge}입니다.

우리가 "변수 업데이트 규칙" 파트에서 남겨두었던 문제를 기억하시나요?
변수 업데이트 규칙은 캐릭터 정의 이전이나 **D4**처럼 주의력을 덜 차지하는 위치에 배치하여, AI가 다른 프롬프트와 스토리에 더 집중하게 만들 수 있습니다. 하지만 AI가 변수 업데이트 규칙에 주의를 덜 기울이게 되면, 변수를 업데이트할 때 오류가 발생하기 더 쉬워집니다.

변수 업데이트 규칙의 핵심은 [check]{.badge}입니다. 이것은 AI에게 **무엇을 근거로 변수를 업데이트해야 하는지, 어떤 값으로 업데이트해야 하는지** 알려줍니다. 우리는 변수 출력 형식의 `<Analysis>` 부분에서 AI에게 **생각의 사슬(CoT)**을 통해 [check]{.badge}를 상기하도록 요구했습니다. 즉, 변수 업데이트 규칙을 다시 "**소환(Recall)**"하여 변수 업데이트를 안정화하는 것입니다.

## 태번 정규식 (Tavern Regex)

변수 출력 형식을 갖추었으므로, 이제 AI는 답장을 할 때 `<UpdateVariable>`을 출력하여 변수를 업데이트할 것입니다. 하지만 이 출력값은 메시지 기록에 남게 되며, 우리가 별도로 처리하지 않으면 이후 플레이할 때 태번(SillyTavern)은 모든 메시지의 `<UpdateVariable>`을 다시 AI에게 전송하게 됩니다.

하지만 분명한 건, AI가 출력한 `<UpdateVariable>`을 **다시 AI에게 보낼 필요는 없다**는 점입니다. `<UpdateVariable>` 내의 업데이트 명령은 이미 MVU 스크립트가 해석하여 사용했으며, AI가 이후 답장을 작성할 때 변수 업데이트 형식을 안정화하기 위해 이것을 참고할 필요는 거의 없기 때문입니다. (우리는 이미 [월드 인포]{.badge}에 변수 출력 형식을 제공했습니다.)

만약 우리가 모든 메시지의 `<UpdateVariable>`을 남겨서 AI에게 보낸다면:

* 첫째, **토큰**이 낭비됩니다.
* 둘째, AI가 불필요하게 이전의 `<UpdateVariable>`을 학습하는 데 주의력을 낭비하여, **스토리에 집중하는 비중이 줄어들 수 있습니다.**
* 마지막으로, AI가 게을러져서 실제로 변수를 어떻게 업데이트할지 분석하지 않고 **이전의 `<UpdateVariable>`을 그대로 베껴 쓸 가능성**이 있습니다!

따라서 우리는 이후 생성 시 `<UpdateVariable>` 부분을 AI에게 보내지 않도록 해야 합니다. 이때 사용하는 것이 바로 **태번 정규식**입니다.

태번 정규식은 AI의 답장과 사용자의 입력 중에서 특정 텍스트를 포착하여, 특정 용도에 따라 지정된 내용으로 치환해 줍니다.

* [프롬프트 전용 (Script Only)]{.badge}: AI에게 전송될 때만 지정된 내용으로 치환됩니다.
* [GUI 전용 (Display Only)]{.badge}: 태번 화면에 표시될 때만 지정된 내용으로 치환됩니다.
* **둘 다 체크 해제**: AI 출력을 수신할 때나 사용자 입력을 전송할 때 **영구적으로 치환**됩니다.

<p align="center">
  <img src="https://vwdygpvfycixugqsgwgn.supabase.co/storage/v1/object/public/post-images/1980dc04-3f08-4581-88cb-b949a10d585b/031a7caf-1590-41cd-9ca7-81ae99b945f7.jpg" width="600">
</p>

!!! tip "팁"
    만약 위의 설명이 잘 이해되지 않는다면, 본인의 태번(SillyTavern)에 기본 포함된 정규식 세트나 다른 캐릭터 카드에 포함된 미화용 정규식을 살펴보는 것도 좋은 방법입니다.

여러분의 편의를 위해, 칭콩리(青空莉)가 `<UpdateVariable>` 변수 업데이트를 처리하는 **태번 정규식**을 미리 만들어 두었습니다. 다음 세 가지 버전 중 하나를 선택해 다운로드하여 가져오기(Import)만 하면 됩니다.

* **미화판 (Beautify)** ([<u>데모 보기</u>](https://gitgud.io/StageDog/tavern_resource/-/raw/main/src/%E6%AD%A3%E5%88%99/%E5%8F%98%E9%87%8F%E6%9B%B4%E6%96%B0/%E7%BE%8E%E5%8C%96%E7%89%88.mp4)): <a href="../../assets/files/regex-remove-variable-update.json" download="regex-[不发送]去除变量更新.json">`[전송 안 함] 변수 업데이트 제거`</a>, `[미화] 변수 업데이트 중`, `[미화] 전체 변수 업데이트`
* **접기판 (Fold)** ([데모 보기](https://gitgud.io/StageDog/tavern_resource/-/raw/main/src/%E6%AD%A3%E5%88%99/%E5%8F%98%E9%87%8F%E6%9B%B4%E6%96%B0/%E6%8A%98%E5%8F%A0%E7%89%88.mp4)): `[전송 안 함] 변수 업데이트 제거`, `[접기] 변수 업데이트 중`, `[접기] 전체 변수 업데이트`
* **알림판 (Hint Only)** (내용 펼치기 불가): `[전송 안 함] 변수 업데이트 제거`, `[알림] 변수 업데이트 중`, `[알림] 전체 변수 업데이트`

여기서 각 정규식의 역할은 다음과 같습니다.

* [변수 업데이트 제거]{.badge}는 정규식의 [프롬프트 전용 (Script Only)]{.badge} 기능을 활용하여, AI에게 전송할 때 `<UpdateVariable>` 부분 전체를 **공백으로 치환**합니다.
* [변수 업데이트 중]{.badge}과 [전체 변수 업데이트]{.badge}는 정규식의 [GUI 전용 (Display Only)]{.badge} 기능을 활용하여, `<UpdateVariable>`의 표시 방식을 예쁘게 꾸며줍니다. 이를 통해 플레이어는 업데이트 발생 여부는 알 수 있지만, 구체적으로 무엇이 업데이트되었는지는 숨길 수 있습니다.

---

하지만, `변수 업데이트 제거`는 기본적으로 **모든 메시지**의 `<UpdateVariable>`을 AI에게 보내지 않게 설정되어 있습니다. 때로는 **마지막 한두 개의 메시지는 AI에게 보내도록 남겨두는 것**이 좋은 선택일 수 있습니다.

만약 모든 메시지의 `<UpdateVariable>`을 AI에게 보내지 않는다면, AI는 **모든 과거 줄거리**를 참고해 변수를 업데이트해야 한다고 착각하여, **이미 처리된 업데이트를 반복**할 수 있기 때문입니다.

> **예시:**
> 지난 답장에서 AI가 "하쿠아와 주인공이 데이트함"이라는 줄거리에 근거해 이미 **하쿠아의 의존도**를 업데이트했습니다.
> 하지만 이번 답장을 작성할 때 이전의 `<UpdateVariable>`(업데이트 기록)을 볼 수 없다면, AI는 "어? 데이트를 했네? 점수 올려야지!"라고 생각하고 다시 한번 의존도를 올릴 수 있습니다.

만약 이렇게 줄거리에 근거해 변수를 중복 업데이트하는 상황을 겪는다면, **마지막 1~2개 메시지의 변수 업데이트 내역은 AI에게 보내도록 유지**하는 선택을 할 수 있습니다.

이를 구현하기 위해 태번 정규식의 최소 깊이 기능을 사용할 수 있습니다. 이 기능은 정규식이 **최소한 특정 수치 이상의 깊이(오래된 정도)를 가진 메시지**에만 작동하도록 제한합니다.

```text
...
깊이 3: 뒤에서 4번째 메시지, 보통 AI의 답장
깊이 2: 뒤에서 3번째 메시지, 보통 사용자의 입력
깊이 1: 뒤에서 2번째 메시지, 보통 마지막 AI의 답장
깊이 0: 뒤에서 1번째 메시지, 보통 마지막 사용자의 입력
```

다시 말해, 만약 우리가 [변수 업데이트 제거]{.badge}의 최소 깊이(Min Depth)를 4로 설정한다면, 이 정규식은 뒤에서 5번째 메시지 및 그보다 더 오래된 메시지에만 유효하게 됩니다. 이렇게 하면 우리는 마지막 한두 번의 변수 업데이트 현황을 AI에게 전달할 수 있게 되는 것이죠!

!!! failure "물론 대가는 따릅니다" 앞서 언급했듯이 토큰 낭비, 주의력 낭비, 그리고 AI가 실제로 변수를 어떻게 업데이트할지 분석하고 고민하는 대신 이전의 <UpdateVariable>을 그대로 베껴 쓰는 게으름을 피울 가능성이 생깁니다. 😭

변수 업데이트 강조
우리가 D0(Author's Note 등)에 변수 출력 형식을 설정해 두더라도, 일부 모델이나 프리셋은 여전히 요구사항대로 <UpdateVariable>...</UpdateVariable>을 출력하여 변수를 업데이트하지 않을 수 있습니다.

만약 이런 상황을 겪는다면, 칭콩리(青空莉)가 **추가 출력 형식**에서 제안했던 것처럼, D0의 변수 출력 형식에 출력 형식 강조를 추가할 수 있습니다.
...
# 서로 다른 오프닝에 서로 다른 변수 초기값 설정 (증분 방식)

앞서 **[서로 다른 오프닝에 서로 다른 변수 초기값 설정 (전체 방식)](#)**에서, 저는 `<initvar>...</initvar>` 블록을 사용하여 오프닝마다 다른 변수 초기값을 설정하는 방법을 소개했습니다.

하지만 이 방법은 **각 오프닝마다 변수 초기값 전체를 다시 작성해야 한다**는 단점이 있습니다.
예를 들어, 분명 `[initvar]` 항목과 **오프닝 2**가 단지 [좌절 모드 스위치]{.badge} 하나만 다르더라도, 우리는 **오프닝 2**에 전체 변수 구조를 똑같이 작성해야만 했습니다.
...
너무 번거롭죠! 어떨 때는 그저 변수 한두 개만 바꾸고 싶을 뿐인데 말이죠. (qwq)

......`<initvar>...</initvar>` 블록을 사용하여 오프닝에서 변수 초기값을 완전히 새로 설정하는 방법 외에도, 우리는 `[initvar]` 항목에 설정된 **변수 초기값을 부분적으로 수정**할 수도 있습니다.

이를 위해서는 **변수 출력 형식**이 어떻게 적용되는지 다시 한번 이해할 필요가 있습니다.

* AI는 변수 출력 형식에 따라, 우선 **변수 업데이트에 대한 분석**을 출력한 뒤, **실제 변수 업데이트 명령**을 출력합니다.
* MVU는 메시지 속의 변수 업데이트 명령을 읽고, 대응하는 변수를 업데이트합니다.
* 이 업데이트 과정은 **모든 메시지(Floor)**에서 진행됩니다:
    `[initvar]` 설정값 $\rightarrow$ `0번 메시지` 업데이트 후 값 $\rightarrow$ `1번 메시지` 업데이트 후 값 $\rightarrow$ ...
* 각 메시지는 자신의 변수 값을 저장합니다. 따라서 [태번 입력창 왼쪽 요술봉 ‣ 변수 관리자 ‣ 메시지(Message Floor)]{.badge}에서 각 메시지(층)의 변수 값을 확인할 수 있으며, 메시지를 삭제할 때 변수 또한 정상적으로 되돌려집니다.

다시 한번 강조하자면, **"이 업데이트 과정은 모든 메시지에서 진행됩니다"**.
맞습니다! **오프닝 메시지(0번 메시지)**에서도 변수 업데이트 명령을 사용하여 변수를 갱신할 수 있습니다!
...
<JSONPatch>
[
  { "op": "replace", "path": "/시라카와 리/좌절 시나리오 스위치", "value": true }
]
</JSONPatch>
</UpdateVariable>
```

하지만 <JSONPatch> 부분이 너무 복잡해 보이나요? 걱정 마세요! 우리에겐 이미 변수 출력 형식이 있습니다. 오프닝 스토리를 AI에게 보내서, AI가 알아서 대응하는 변수 업데이트 명령을 생성하게 시키면 됩니다.

## 두 가지 변수 업데이트 방식 지원

AI가 줄거리(스토리)에 집중할 수 있도록, MVU는 플레이어가 자유롭게 선택할 수 있는 **두 가지 변수 업데이트 방식**을 지원합니다.

* **[AI 출력에 포함 (Along with AI Output)]{.badge}**: AI가 줄거리를 먼저 출력한 뒤, 이어서 변수 업데이트 명령을 출력합니다.
* **[추가 모델 해석 (Extra Model Analysis)]{.badge}**: 하나의 AI가 전문적으로 줄거리를 출력하고, 또 다른 별개의 AI가 그 줄거리를 해석하여 변수 업데이트 명령을 전문적으로 출력합니다.

<p align="center">
  <img src="https://vwdygpvfycixugqsgwgn.supabase.co/storage/v1/object/public/post-images/1980dc04-3f08-4581-88cb-b949a10d585b/e69caa1a-05de-48ec-a595-92e743bf20ac.jpg" width="600">
</p>

만약 여러분이 '문의 주인 카드 작성 도우미(门之主写卡助手)'를 통해 3대 변수 프롬프트를 생성했다면, 생성된 항목의 이름이 `변수 목록`, `[mvu_update]변수 업데이트 규칙`, `[mvu_update]변수 출력 형식`인 것을 눈치채셨을 겁니다.

`[mvu_update]`라는 접두사는 사실 이 **두 가지 변수 업데이트 방식**을 모두 지원하기 위해 존재하는 것입니다.

* **[AI 출력에 포함]{.badge} 방식:** 모든 항목이 정상적으로 AI에게 전송됩니다.
* **[추가 모델 해석]{.badge} 방식:**
    * 이름에 `[mvu_plot]`이 포함된 항목은 **줄거리 담당 AI**에게만 전송됩니다.
    * 이름에 `[mvu_update]`가 포함된 항목은 **변수 업데이트 담당 AI**에게만 전송됩니다. (그렇기 때문에 [변수 업데이트 규칙]{.badge}과 [변수 출력 형식]{.badge} 앞에 `[mvu_update]`가 붙은 것입니다.)
    * 이름에 `[mvu_plot]`과 `[mvu_update]`가 모두 없는 항목은 **두 AI 모두에게** 전송됩니다.

즉, 만약 여러분이 이 튜토리얼을 따라 `변수 목록`, `[mvu_update]변수 업데이트 규칙`, `[mvu_update]변수 출력 형식`으로 변수 프롬프트를 작성했다면, **당신의 캐릭터 카드는 이미 두 가지 변수 업데이트 방식을 모두 지원**하는 상태입니다!

!!! tip "팁"
    플레이어로서 게임을 할 때, `[mvu_update]` 또는 `[mvu_plot]`이 이름에 포함된 항목을 넣은 **전역 월드 인포(Global World Info)**를 활성화하여, 줄거리 AI와 변수 업데이트 AI의 행동을 제어할 수도 있습니다.

    예를 들어, `[mvu_update]` D9999 항목을 사용하여 **변수 업데이트 AI에게 탈옥 프롬프트를 추가**할 수 있습니다.

변수 프롬프트는 그저 프롬프트일 뿐입니다

변수 목록이든, 변수 업데이트 규칙이든, 혹은 변수 출력 형식이든 간에, 이것들은 모두 우리가 [월드 인포]{.badge}에 작성해 넣는 **프롬프트에 불과**합니다.
위의 튜토리얼에서 저는 **'문의 주인 카드 작성 도우미(门之主写卡助手)'**가 사용하는 버전을 예시로 들었지만, 여러분은 얼마든지 **자신만의 세트**를 직접 설계할 수 있습니다!

또한, 어떤 변수들은 **특정 상황에서만** 필요할 수도 있습니다.
예를 들어 만약 **임무 시스템(Task System)**이 특정 캐릭터와만 묶여 있다면, 당연히 우리는 해당 캐릭터가 등장할 때만 임무 시스템 관련 프롬프트를 전송하면 될 것입니다.

따라서 `{{format_message_variable::stat_data}}`를 사용하여 뭉뚱그려 보내는 대신, 다음과 같이 나누어 작성할 수 있습니다.
...
# ⬇️⬇️⬇️⬇️⬇️ 초록불(Green Light)이나 추후 소개할 기능을 이용하여,
# 우리는 특정 상황에서만 아래 프롬프트를 전송하도록 설정할 수 있습니다.
임무 시스템:
...
임무 시스템의 변수 업데이트 규칙 또한 동일한 방식으로 처리할 수 있습니다.

변수 프롬프트 테스트

이제 변수 프롬프트가 준비되었습니다. **MVU 변수 프레임워크** 설정에서 [변수 업데이트 오류 시 알림]{.badge}을 체크하세요.
그다음 **변수 관리자**를 열어 한쪽에 두고, AI에게 아무 말이나(예: `[스토리 진행]`){.badge} 보내서 AI가 줄거리를 진행하게 한 뒤, 메시지(Message Floor)에 `<UpdateVariable>` 블록이 올바르게 출력되는지 확인해 보세요.

<p align="center">
  <img src="https://vwdygpvfycixugqsgwgn.supabase.co/storage/v1/object/public/post-images/1980dc04-3f08-4581-88cb-b949a10d585b/3dc6d008-444b-4809-8fc1-37d8b6f40feb.jpg" width="600">
</p>

### 문제 해결 가이드

!!! failure "<UpdateVariable> 블록이 출력되지 않는 경우"
    다른 프롬프트가 주의력(Attention)을 너무 많이 차지하고 있거나, 변수 프롬프트가 올바르게 적용되지 않았을 가능성이 있습니다.

    1. **전역 월드 인포(Global World Info)**를 비활성화해 보세요.
    2. 다른 프롬프트의 주의력을 조정한 뒤, 본 챕터의 내용대로 변수 프롬프트가 정확히 설정되었는지 검증하세요.

!!! warning "노란색 에러 발생: `[MVU]发生变量更新错误`"
    만약 `[MVU zod]`가 아닌 `[MVU] 변수 업데이트 오류 발생([MVU]发生变量更新错误)`이라는 노란색 에러가 뜬다면, 이는 **변수 구조 스크립트**에 오류가 있다는 뜻입니다.

    **해결 방법:**
    1. 변수 구조 스크립트를 껐다 켜보세요.
    2. 로그를 확인하여 **빨간색 에러**가 있는지 찾아보세요.
        * **PC:** `[F12 ‣ 콘솔 (devtool)]{.badge}`
        * **모바일:** `[입력창 좌측 하단 요술봉 ‣ 로그 뷰어 (Log Viewer)]{.badge}`
    3. 빨간색 에러가 있다면 그 내용을 복사해서 AI에게 보내 **변수 구조 스크립트를 다시 작성**하게 시키세요. (에러가 없더라도 그냥 다시 작성하게 시켜도 됩니다.)
    4. 도저히 해결되지 않는다면 포럼 등을 통해 원작자(칭콩리, qwq)에게 문의하세요.

!!! success "테스트 성공"
    위의 문제가 발생하지 않았다면, 일반적으로 변수 프롬프트가 정상적으로 작동하고 있는 것입니다!
    변수가 업데이트될 때 변수 관리자에서 하이라이트 애니메이션이 재생되고, 업데이트된 프롬프트 내용이 표시될 것입니다.


## 되돌아보기

이로써 우리의 변수 환경 구축이 완료되었습니다.

* 우리는 **변수 구조 스크립트**를 생성하고 새로 만들었으며, 이는 우리 캐릭터 카드에 어떤 변수가 있는지 정의했습니다.
* 우리는 `[initvar]변수 초기화 활성화 금지` 항목과 오프닝의 `<initvar>` 블록을 사용하여 각 오프닝마다 초기값을 설정했습니다.
* 우리는 AI가 변수를 이해할 수 있도록 **변수 프롬프트**를 설계했습니다.
    * **변수 목록**을 통해 현재의 변수 현황을 파악했습니다.
    * **변수 업데이트 규칙**을 통해 변수가 어떤 상황에서 업데이트되어야 하는지, 그리고 어떤 값으로 업데이트되어야 하는지 파악했습니다.
    * **변수 출력 형식**을 통해 변수를 업데이트하기 위해 무엇을 출력해야 하는지 파악했습니다.
* **태번 정규식** 세트를 가져와서, 이전의 `<UpdateVariable>` 블록이 다시 AI에게 전송되지 않도록 하여 **AI 과적합(Overfitting)**을 방지했습니다.

이 환경은 매번 모든 텍스트를 전체 출력해야 했던 기존의 전통적인 상태창을 효과적으로 대체하며, AI에게 현재의 스토리 상태를 정확하게 상기시켜 줄 수 있습니다!

——하지만 변수가 할 수 있는 건 이뿐만이 아닙니다.

앞서 언급했듯이, `하쿠아.의존도`의 경우 AI가 이름만 보고도 이것이 하쿠아의 주인공에 대한 애착 정도라는 것은 알 수 있습니다. 하지만 **23**과 같은 구체적인 수치에서, 하쿠아는 어떤 심리 상태여야 할까요? 어떤 행동을 취하려 할까요? 이런 것들은 우리의 추가적인 해석이 필요합니다.

당시 우리의 선택은 **캐릭터 단계** 항목을 새로 만들어, 각 의존도 단계별 하쿠아의 행동을 나열하는 것이었습니다.

```yaml
하쿠아의 현재 행동: # 하쿠아의 현재 의존도는 {{format_message_variable::stat_data.하쿠아.의존도}}이므로, 다음 예시와 유사한 행동을 취하는 경향이 있음
  # 0~19 일 때
  소극적 자기 파괴:
    ...
  # 20~39 일 때
...
하지만 이 방식에는 명백한 문제가 있습니다. 비록 현재 하쿠아의 의존도는 23이지만, 우리는 여전히 0~19, 40~59 등 다른 단계의 프롬프트까지 모두 전송하고 있습니다.

이는 입력 토큰을 낭비할 뿐만 아니라, AI의 주의력을 불필요하게 차지하여, AI가 하쿠아에게 어느 단계의 행동을 취하게 해야 할지 혼란스럽게 만들 수 있습니다.

만약 현재 의존도에 근거하여, 현재 의존도에 대응하는 프롬프트만 전송할 수 있다면 얼마나 좋을까요! 만약 의존도가 100에 도달했을 때, 미리 작성해 둔 고백 이벤트를 트리거할 수 있다면 얼마나 좋을까요!

......물론, 우리는 할 수 있습니다😈 다음 페이지에서 확인하세요.