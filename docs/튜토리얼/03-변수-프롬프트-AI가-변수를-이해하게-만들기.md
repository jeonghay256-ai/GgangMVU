    ...
    주인공:
      아이템란:
        낡은_반창고:
          ...
          수량: 1
        박하사탕:
          설명: 정신 차리기용 강력한 박하사탕, 예전에 그녀는 이 맛을 매우 싫어했어
          수량: 1
    </status_current_variable>
    ```
...
=== "프롬프트 뷰어"
    ...
    <p align="center">
      <img src="https://stagedog.github.io/_images/%E6%8F%90%E7%A4%BA%E8%AF%8D%E6%9F%A5%E7%9C%8B%E5%99%A8.png" width="500">
    </p>
...
<p align="center">
  <img src="https://vwdygpvfycixugqsgwgn.supabase.co/storage/v1/object/public/post-images/1980dc04-3f08-4581-88cb-b949a10d585b/83bbaf6d-6b47-4654-821f-dcd0d716cc1b.png" width="500">
</p>
...
예상대로 AI 응답에 `2024-04-08 10:45`라는 시간이 나타났습니다!
...
변수 목록은 변수의 현재 값을 나열하지만, AI는 여전히 변수 목록의 값이 최신 스토리가 아니라 이전 스토리에 해당한다고 생각할 수 있습니다. 이것은 월드인포 항목의 삽입 위치 때문입니다.

일반적으로 프롬프트는 다음 순서로 프롬프트를 AI에게 전송합니다:
...
AI가 변수 목록이 **최신 스토리(즉, 최신 AI 응답)**에 해당한다는 것을 직접 이해하게 하려면, 변수 목록을 반드시 **D1** 또는 **D0** 위치에 설정해야 합니다.

!!! tip "해설: Depth(깊이)와 위치 선정"
    SillyTavern에서 **Depth(깊이)**는 채팅 로그의 끝(최신)에서부터 몇 번째 위에 있는지를 나타냅니다.
    
    * **D0:** 가장 최신 메시지 (보통 방금 내가 보낸 입력)
    * **D1:** 바로 직전 메시지 (보통 직전 AI의 응답)
    
    변수 목록을 **D1**이나 **D0** 같은 최하단에 배치하는 이유는, LLM이 텍스트를 위에서 아래로 읽을 때 **가장 마지막에 읽은 정보를 가장 중요하게 인식하는 경향**이 있기 때문입니다.
...
하지만 이 방식에는 분명한 문제가 있습니다. 현재 하쿠아의 의존도가 **23**임에도 불구하고, 우리는 여전히 **0~19, 40~59 등 해당하지 않는 단계의 프롬프트까지 모두 전송**하고 있다는 점입니다.
    
이는 입력 토큰을 낭비할 뿐만 아니라, AI의 주의력(Attention)을 분산시켜 AI가 하쿠아에게 어떤 단계의 행동을 취하게 해야 할지 혼란을 줄 수 있습니다.
...
??? note "생성된 변수 업데이트 규칙 (클릭하여 펼치기)"
    ...
    변수 업데이트 규칙:
      세계:
        현재 시간:
          format: YYYY년 MM월 DD일 요일X HH:MM
        최근 사건:
          type: |-
            {
          ...
          check:
            - 완료해야 할 임무, 약속, 중요 사건 등을 기록
            - 완료 후 목록에서 제거, 새로운 사건 발생 시 즉시 추가
            - 활성 사건은 최대 5~8개로 유지하여 과도한 복잡성 방지
      하쿠아:
        의존도:
          type: number
          range: 0~100
          check:
            - <user>의 행동에 대한 하쿠아의 인지와 반응에 따라 ±(3~6) 조정
            - 하쿠아가 현재 <user>의 행동을 알아챘을 때만 업데이트
        복장.${상의|하의|속옷|양말|신발|장식}:
          check:
            - 환복, 의복 손상, 특수 상황 시 업데이트
            - 설명에는 색상, 재질, 스타일 등의 세부 정보 포함
            - 하쿠아의 심리 상태와 <user>에 대한 관심도 반영
        칭호:
          type: |-
            {
              [칭호명: string]: {
                효과: string;
                자가 평가?: string;  // 기본값은 '평가 대기'
              }
            }
          check:
            - 하쿠아의 중요한 행동, 심리 변화 또는 <user>와의 상호작용을 기반으로 획득
            - 자가 평가가 '평가 대기'라면 즉시 자가 평가 업데이트
            - 칭호는 하쿠아의 현재 의존 상태와 심리적 발전을 반영해야 함
            - 최대 Math.ceil(의존도/10)개의 칭호 유지, 초과 시 가장 오래된 것 제거
      주인공:
        아이템란:
          type: |-
            {
...
- **`range`: 숫자 변수가 있어야 하는 범위**
    예를 들어 `하쿠아.의존도`는 반드시 0~100 사이여야 합니다. 이것도 변수 구조에서 규정했지만, AI에게 알려주면 더 좋습니다:AI가 의존도를 `128`로 업데이트했다가 강제로 `100`으로 수정되는 것보다는, AI가 처음부터 `0~100` 사이여야 함을 알고 적절한 값으로 계산하는 것이 더 바람직하기 때문입니다.

- **`format`: 변수가 만족해야 하는 특정 형식**
    예를 들어 `세계.현재시간`은 반드시 `YYYY년MM월DD일 요일 HH:MM` 형식이어야 합니다.
    변수 구조에서 강제할 수도 있지만, 예시 카드에서는 그렇게 하지 않고 변수 업데이트 규칙에 `format` 한 줄을 적어 AI에게 알리기만 했습니다. 
    * 왜냐하면 `월드.현재 시간`은 AI가 현재 시간을 인지하기 위한 용도이므로 형식이 완벽하지 않아도(예: `HH:MM YYYY/MM/DD`) 기능을 수행하는 데 문제가 없기 때문입니다.

- **`check`: AI가 변수를 업데이트할 때 고려해야 하는 논리적 요소**
    예를 들어 `하쿠아.의존도`는 `하쿠아가 <user> 행동에 대한 인식과 반응에 따라 조정`해야 하고, `(3~6)`이며, `하쿠아.칭호`는 `하쿠아의 중요한 행동, 심리 변화 또는 <user>와의 상호작용을 기반으로 획득`해야 합니다. 사실상 이 부분이 핵심입니다. 변수가 **어떤 상황에서 업데이트되어야 하는지**, **어떤 값으로 변해야 하는지**를 자연어로 상세히 서술하면 됩니다.
...
=== "한글 프롬프트 (번역)"
    ...
    ```yaml
    # 이해를 돕기 위해 주석 및 플레이스홀더를 한국어로 번역한 버전입니다.
    변수_출력_형식:
      rule:
        - 반드시 답변 마지막에 업데이트 분석과 실제 업데이트 명령을 한 번에 출력해야 함
        - 업데이트 명령 효과는 **JSON Patch (RFC 6902)** 표준과 유사함. 유효한 JSON 배열이어야 하며 표준 대신 다음 작업을 지원함:
...
=== "AI 출력 결과 예시"
    ...
    - 월드 인포.최근 사건: "转学生安置" (전학생 적응) is ongoing but the specific interaction happened. "午休临近" (점심시간 임박) is becoming more relevant as break continues.
    </Analysis>
    <JSONPatch>
    [
...
AI가 변수를 업데이트하기 전에 거치는 사고 과정의 세부 단계입니다.
...
=== "영문 프롬프트"
    ...
    **스토리 진행에 따른 변수의 급격한 변동 허용 여부**
    ```yaml
    - ${decide whether dramatic updates are allowed as it's in a special case or the time passed is more than usual: yes/no}
    ```

    **각 변수에 설정된 `check` 규칙을 상기하고, 이를 바탕으로 업데이트 방향을 분석합니다**
    ```yaml
    - ${analyze every variable based on its corresponding `check`, according only to current reply instead of previous plots: ...}
    ```
...
=== "한글 프롬프트 (번역)"
    ...
    **스토리 진행에 따른 변수의 급격한 변동 허용 여부** (예: 호감도 100 $\rightarrow$ 0)
    ```yaml
    - ${현재 전개가 충분히 특수한지, 시간 경과가 정상 범위를 훨씬 초과했는지에 따라 변수값의 극적인 변화 허용 여부 판단: 예/아니오}
    ```
...
<p align="center">
  <img src="https://vwdygpvfycixugqsgwgn.supabase.co/storage/v1/object/public/post-images/1980dc04-3f08-4581-88cb-b949a10d585b/031a7caf-1590-41cd-9ca7-81ae99b945f7.jpg" width="500">
</p>
...
이런 상황이 발생하면, 청공리가 **[<U>추가 출력 형식</U>](https://stagedog.github.io/%E9%9D%92%E7%A9%BA%E8%8E%89/%E5%B7%A5%E5%85%B7%E7%BB%8F%E9%AA%8C/%E6%8F%90%E7%A4%BA%E8%AF%8D%E4%B8%AA%E4%BA%BA%E5%86%99%E6%B3%95/%E9%A2%9D%E5%A4%96%E8%BE%93%E5%87%BA%E6%A0%BC%E5%BC%8F/)**에서 언급한 것처럼, D0에 변수 출력 형식에 출력 형식 강조를 추가할 수 있습니다:
...
너무 번거롭죠! 변수 딱 하나만 바꾸고 싶을 뿐인데... `<initvar>...</initvar>` 블록으로 변수 초기값을 완전히 다시 설정하는 것 외에도, `[initvar]` 항목에서 설정한 변수 초기값을 부분적으로만 수정할 수도 있습니다.
...
<JSONPatch>
[
  { "op": "replace", "path": "/시라카와리/좌절 스토리 스위치", "value": true }
]
</JSONPatch>
</UpdateVariable>
```
...
변수 목록, 변수 업데이트 규칙, 변수 출력 형식 등은 모두 우리가 **월드 인포(World Info)**에 작성하는 프롬프트의 일부일 뿐입니다.

위 튜토리얼에서는 문지기(Gatekeeper) 캐릭터 작성 도우미가 사용하는 버전을 제시했지만, 여러분이 직접 독자적인 버전을 설계해도 완전히 무방합니다!

또한, 일부 변수는 특정 상황에서만 필요할 수도 있습니다. 예를 들어 '임무 시스템'이 특정 캐릭터와만 연관되어 있다면, 당연히 해당 캐릭터가 등장할 때만 임무 시스템 관련 프롬프트를 전송하는 것이 효율적입니다.

따라서 단순히 `{{format_message_variable::stat_data}}`로 모든 데이터를 뭉뚱그려 보내는 대신, 다음과 같이 나누어 작성할 수 있습니다.

```yaml
<status_current_variable>
캐릭터:
  {{format_message_variable::stat_data.캐릭터}}
# ⬇️⬇️⬇️⬇️⬇️ Green Light(그린 라이트)나 추후 소개할 기능을 활용하여, 
# 특정 상황에서만 아래 프롬프트를 전송하도록 설정할 수 있습니다.
임무 시스템:
  {{format_message_variable::stat_data.임무_시스템}}
# ⬆️⬆️⬆️⬆️⬆️
</status_current_variable>
...
```yaml
하쿠아의 현재 행동: 
  # 하쿠아의 현재 의존도는 {{format_message_variable::stat_data.하쿠아.의존도}}이므로, 다음 예시와 유사한 행동을 하는 경향이 있음
  # 0~19 일 때
  소극적 자기 파괴:
    ...
  # 20~39 일 때
...
!!! warning "모든 단계의 프롬프트를 전송할 때의 문제점"
    하지만 이 방식에는 분명한 문제가 있습니다. 
    
    현재 하쿠아의 의존도가 **23**임에도 불구하고, 우리는 여전히 **0~19, 40~59 등 해당하지 않는 단계의 프롬프트까지 모두 전송**하고 있다는 점입니다. 
    
    이는 입력 토큰을 낭비할 뿐만 아니라, AI의 주의력을 분산시켜 AI가 하쿠아에게 어떤 단계의 행동을 취하게 해야 할지 혼란을 줄 수 있습니다.

만약 현재 의존도에 딱 맞춰서, **그에 해당하는 프롬프트만 전송**할 수 있다면 얼마나 좋을까요!

만약 의존도가 100에 도달했을 때, **미리 작성해 둔 고백 이벤트**를 딱 트리거할 수 있다면 얼마나 좋을까요! ...물론, **가능합니다.** 😈
다음 페이지에서 설명드리겠습니다.