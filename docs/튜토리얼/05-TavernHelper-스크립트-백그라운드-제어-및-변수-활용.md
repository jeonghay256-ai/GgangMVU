...
또 다른 방법으로는 `injectPrompts`의 `filter` 필드를 활용하여, 주어진 조건이 충족될 때만 사용되는 프롬프트를 직접 주입하는 것입니다. 예제 카드의 즉시 이벤트 스크립트가 바로 이 방식을 사용합니다:
...
injectPrompts([
 {
   ...
   depth: 0,
   role: 'system',
   content: '【【의존도 최저 시 즉시 이벤트】】',

   // `getAllVariables()`로 최신 변수를 가져오고, 그중 하쿠아 의존도가 0일 때만 이 프롬프트를 활성화
   filter: () => _.get(getAllVariables(), 'stat_data.하쿠아.의존도') === 0,
...
물론, `injectPrompts`는 녹색 불 활성화 전용 프롬프트를 주입하는 것 외에도, **Dn 프롬프트를 직접 주입**할 수도 있습니다. 자세한 내용은 **[Tavern이 World Book을 처리하는 방법](https://stagedog.github.io/%E9%9D%92%E7%A9%BA%E8%8E%89/%E5%B7%A5%E5%85%B7%E7%BB%8F%E9%AA%8C/%E9%85%92%E9%A6%86%E5%A6%82%E4%BD%95%E5%A4%84%E7%90%86%E4%B8%96%E7%95%8C%E4%B9%A6/)**을 참고해 주세요.

## 코드에서 AI 생성 요청 및 결과로 변수 업데이트

물론 프론트엔드 인터페이스나 스크립트에서 직접 AI 생성을 요청할 수도 있습니다. 생성 결과에 `<JSONPatch>` 등의 변수 업데이트 명령이 포함되어 있다면, 이를 파싱하여 변수를 업데이트할 수 있습니다.
...
// AI 생성 요청
const content = await generate({ user_input: '안녕하세요' });

// 생성 결과 파싱
const new_data = await Mvu.parseMessage(content, old_data);

// 업데이트된 변수를 해당 메시지에 다시 쓰기
await Mvu.replaceMvuData(new_data, { type: 'message', message_id: getCurrentMessageId() });
```

자연스럽게, 이 방식을 활용하면 플레이어가 인터페이스 내에서 직접 AI와 대화할 수 있도록 만들 수 있습니다. 자세한 내용은 **[실시간 프론트엔드 인터페이스 또는 스크립트 작성](https://stagedog.github.io/%E9%9D%92%E7%A9%BA%E8%8E%89/%E5%B7%A5%E5%85%B7%E7%BB%8F%E9%AA%8C/%E5%AE%9E%E6%97%B6%E7%BC%96%E5%86%99%E5%89%8D%E7%AB%AF%E7%95%8C%E9%9D%A2%E6%88%96%E8%84%9A%E6%9C%AC/)**을 참고해 주세요.

## 스크립트로 더 많은 기능 구현하기

**[실시간 프론트엔드 인터페이스 또는 스크립트 작성](https://stagedog.github.io/%E9%9D%92%E7%A9%BA%E8%8E%89/%E5%B7%A5%E5%85%B7%E7%BB%8F%E9%AA%8C/%E5%AE%9E%E6%97%B6%E7%BC%96%E5%86%99%E5%89%8D%E7%AB%AF%E7%95%8C%E9%9D%A2%E6%88%96%E8%84%9A%E6%9C%AC/)**을 읽은 후, 작성 템플릿 폴더에 있는 `@types/iframe/exported.mvu.d.ts` 파일과 여기 있는 예제 코드를 AI에게 보내서 대신 작성하게 시키면 됩니다!
